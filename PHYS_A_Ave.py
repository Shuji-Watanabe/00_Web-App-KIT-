...
def Cal_Acceleration_Ave(Cal_Ave_Num_Dim,Cal_Ave_Error_Str,Cal_Acceleration_App_num):
    ...
    if Method_Cal_dv[Select_cal_dv]==0:
        ...
        for i in range(Num_Dim):
            with CB0_col_V[i+1]:
                CB0_str_r_i.append( "変化前の速度の第"+str(int(i)+1)+"成分(App%2d)"%(App_num))
                CB0_r_i.append(st.text_input(CB0_str_r_i[i]))
                CB0_str_r_f.append( "変化後の速度の第"+str(int(i)+1)+"成分(App%2d)"%(App_num))
        ...
        for i in range(Num_Dim):
            with CB0_col_V[i+1]:
                CB0_str_r_d.append( "速度の変化量の第"+str(int(i)+1)+"成分(App%2d)"%(App_num))
                try:
